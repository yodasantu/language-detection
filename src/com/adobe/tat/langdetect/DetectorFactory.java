...
public class DetectorFactory {
     ...
     *                             or profile's format is wrong (error code = {@link ErrorCode#FormatError})
     */
    public void loadProfile(List<String> profileDirectory) throws LangDetectException {
        int index = 0;
        int langsize = profileDirectory.size();
        if (langsize < 2)
            throw new LangDetectException(ErrorCode.NeedLoadProfileError, "Need more than 2 profiles");

        for (String json : profileDirectory) {
            try {
    ...
    private Detector createDetector() throws LangDetectException {
        ...
        Detector detector = new Detector(this.wordLangProbMap, this.langlist, this.seed);
        return detector;
    }
